...
public class LuceneReadIndexExample 
{
	...
	public static void main(String[] args) throws Exception 
	{
		...
		SortedSetDocValuesReaderState state = 
				   new DefaultSortedSetDocValuesReaderState(reader, "facet_website");
		FacetsCollector fc = new FacetsCollector();
		QueryParser qp = new QueryParser("firstName", new StandardAnalyzer());
		qp.setAllowLeadingWildcard(true);
		Query firstNameQuery = qp.parse("*a*");
		
		FacetsCollector.search(new IndexSearcher(reader), firstNameQuery, 10, fc);
		
		Facets facets = new SortedSetDocValuesFacetCounts(state, fc);
		FacetResult result = facets.getTopChildren(10, "website");
